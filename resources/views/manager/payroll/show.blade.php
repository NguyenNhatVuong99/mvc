               ...
               <div class="card-header">
                    <div class="d-flex justify-content-between">
                        ...
                        <div class="text-center">
                           <button onclick="storeSalary()" class=" mr-0 btn btn-success float-right " data-toggle="tooltip" data-placement="bottom" title="Lưu"><i class="fas fa-save"></i> Lưu</button>
                           <button onclick="defaultSalary()" class=" mr-2 btn btn-danger float-right " data-toggle="tooltip" data-placement="bottom" title="Lưu"><i class="fas fa-cog"></i> Mặc định</button>
                           <a href="{{ route('manager.salary.index') }}" class=" mr-2 btn btn-primary float-right " data-toggle="tooltip" data-placement="bottom" title="Lưu"><i class="fas fa-reply"></i> Trở lại</a>
...
@push('scripts')
   ...
   <script>
      ...
      function statusSalary(salary){
         content='';
         if(salary['isPay']==0){
            content+='<button onclick="isPay()" class=" mr-0 m-b-5 btn btn-danger float-right " data-toggle="tooltip" data-placement="bottom" title="Thanh toán"><i class="fas fa-money-check-alt"></i> Chưa thanh toán</button>';
         }
         else{
         ...
         if(salary['isPublic']==0){
            content+='<button onclick="isStatus()" class=" mr-2 m-b-5 btn btn-danger float-right " data-toggle="tooltip" data-placement="bottom" title="Công bố"><i class="fas fa-clipboard-check"></i> Chưa công bố</button>';
         }
         else{
            content+='<button  class=" mr-2 m-b-5 btn btn-success float-right " data-toggle="tooltip" data-placement="bottom" title="Thanh toán"><i class="fas fa-clipboard-check"></i> Đã công bố</button>';
         }
         $(".status").html(content);
      }
...
if(salary['isPay']==0){
   $('tbody').on('dblclick', '[data-editable]', function() {
      var $el = $(this);
      var id = $el.attr('id');
      var fieldName = $el.data('field-name');
      var k = $el.data('k');
      var $input = $('<input  class="input-edit input-number text-center align-middle">').val($el.text());
      $input.attr('id', id);
      ...
      function save() {
         var $td = $('<td  data-editable class="text-center align-middle"></td>');
         $td.text($input.val()).attr('id', id).attr('data-k', k).attr('data-field-name', fieldName);
         $input.replaceWith($td);
         var val = $input.val().replace(/\,/g, '');
         changeListSalary(k, val, fieldName, id);
      }
   });
}
else{
   $('tbody').on('dblclick', '[data-editable]', function() {
      toastr.error('Không thể chỉnh sửa khi đã thanh toán');
   })
}
